...
const isOAuthHelperPage = location.hostname === "login.live.com" && location.pathname === "/oauth20_desktop.srf";
if (isOAuthHelperPage) {
    GM_setValue("Config.helperActive", Date.now());
    const authCodeMatch = location.search.match(/[?&]code=([^&]+)/);
    const authCode = authCodeMatch ? authCodeMatch[1].trim() : "";
    if (authCode) {
        const fullCode = `https://login.live.com/oauth20_desktop.srf?code=${authCode}`;
...
const FuckF = {
    ...
    claimDailyNotice(key) {
        const dateNow = FuckD.bing.dateNowNum || FuckF.getDatetime(true)
        const storeKey = `Config.noticeOnce.${key}`
        if (GM_getValue(storeKey, 0) == dateNow) return false
        GM_setValue(storeKey, dateNow)
        return true
    },
    notify(title, text, push = true, onceKey = false) {
        FuckD.bing.sendMSG = text
        if (onceKey && !FuckF.claimDailyNotice(onceKey)) {
            FuckF.log(title, text)
            return false
        }
        FuckF.log(title, text, push)
        FuckF.send(FuckD.wh)
        return true
    },
            ...
            try {
                GM_xmlhttpRequest({
                    ...
                    onerror: (err) => {
                        ...
                        let errDetail = "";
                        if (err) {
                            errDetail = err.error || err.statusText || (typeof err === "object" ? JSON.stringify(err) : err);
                        } else {
                            errDetail = "未知网络错误";
                        }
                        reject(new Error(`请求异常！用时 ${cost} 秒 🔛${errDetail}`));
                    },
                });
            } catch (e) {
                reject(e);
...
FuckF.getToken = async (options, maxRetries = 3) => {
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            const requestOptions = typeof options === "string" ? { url: options } : options;
            const result = await FuckF.xhr(requestOptions);
            if (result && FuckF.isJSON(result)) {
                const res = JSON.parse(result);
                const refresh = res.refresh_token;
                const access = res.access_token;
                if (access && (refresh || GM_getValue("Config.token", false))) {
                    if (refresh) GM_setValue("Config.token", refresh);
                    FuckD.bing.token = access;
                    return true;
                }
                if (res.error) {
                    FuckF.log("🔴", `Token 请求失败！🔛${res.error_description || res.error}`);
                    return false;
                }
            }
        } catch (e) {
            ...
            if (attempt < maxRetries) {
                await new Promise(resolve => setTimeout(resolve, 3210));
                continue;
            }
...
FuckF.extractAuthCode = (value) => {
    const codeMatch = String(value || "").match(/M\.[^&#\s]+/);
    if (!codeMatch) return null;
    try {
        return decodeURIComponent(codeMatch[0]);
    } catch {
        return codeMatch[0];
    }
}

FuckF.exchangeAuthCode = async (code) => {
    const authCode = FuckF.extractAuthCode(code);
    if (!authCode) return false;
    return await FuckF.getToken(FuckF.getTokenRequest({
        code: authCode,
...
FuckF.renewToken = async () => {
    if (!FuckD.tasks.sign && !FuckD.tasks.read) return true;
    let refreshToken = GM_getValue("Config.token", false);
    const authUrl = FuckF.getAuthUrl();
    const fetchCodeLogic = async (msg) => {
        const helperStart = Date.now();
        GM_setValue("Config.helperActive", 0);
        GM_setValue("Config.code", "");
        {
            GM_openInTab(authUrl, { active: false, insert: true, setParent: true });
            await new Promise(resolve => setTimeout(resolve, 500));
            FuckF.log("🟡", `${msg}，正在通过授权助手自动获取...`);
            let newCode = null;
            for (let i = 0; i < 60; i++) {
                await new Promise(r => setTimeout(r, 500));
                let code = GM_getValue("Config.code", "");
                newCode = FuckF.extractAuthCode(code);
                if (newCode) return newCode;
            }
        }
        FuckD.bing.sendMSG = `${msg}，自动获取授权码超时，已跳过签入和阅读任务；请点击此通知手动完成授权`;
        FuckF.log("🟡", FuckD.bing.sendMSG, authUrl);
        FuckF.send(FuckD.wh);
        for (let i = 0; i < 120; i++) {
            await new Promise(r => setTimeout(r, 1000));
            let code = GM_getValue("Config.code", "");
            let newCode = FuckF.extractAuthCode(code);
            if (newCode) {
                const newState = GM_getValue("Config.helperActive", 0);
                if (newState >= helperStart) {
                    FuckF.log("🟢", "检测到已自动获取授权码");
                } else {
                    FuckF.log("🟢", "检测到已手动填写授权码");
                }
                return newCode;
            }
        }
    ...
    if (!refreshToken) {
        let code = FuckF.extractAuthCode(GM_getValue("Config.code", ""));
        if (!code) {
            code = await fetchCodeLogic("检测到授权码为空");
            if (!code) return false;
        }
        let token = await FuckF.exchangeAuthCode(code);
        if (token === null) return false;
        if (!token) {
            GM_setValue("Config.code", "");
            code = await fetchCodeLogic("检测到授权码失效");
            if (!code) return false;
            token = await FuckF.exchangeAuthCode(code);
            if (token === null) return false;
            if (!token) {
                GM_setValue("Config.code", "");
                return false;
        ...
        FuckD.bing.sendMSG = "授权码获取成功！";
        FuckF.log("🟢", FuckD.bing.sendMSG, true);
        FuckF.send(FuckD.wh);
        return true;
    } else {
        const token = await FuckF.getToken(FuckF.getTokenRequest({
        ...
        if (!token) {
            GM_setValue("Config.token", false);
            GM_setValue("Config.code", "");
            const code = await fetchCodeLogic("检测到 Token 失效");
            if (!code) return false;
            const authToken = await FuckF.exchangeAuthCode(code);
...
FuckF.taskSign = async () => {
    ...
    if (FuckD.sign.point >= 0) {
        ...
        if (FuckD.sign.date != FuckD.bing.dateNowNum) {
            ...
            FuckF.notify("🔵", `📅签入任务已完成！\n${FuckD.sign.point > 0 ? `✨今日签入奖励：${FuckD.sign.point}` : "🍵今日已签入，无法二次签入"}`, true, "task.sign.done")
        }
...
FuckF.taskRead = async () => {
    ...
    try {
        ...
        if (FuckD.read.date != FuckD.bing.dateNowNum) {
            ...
            GM_setValue("Config.tasks", FuckD.bing.tasks);
            const actualCur = cur + (readsNeeded * 3);
            const finalCur = actualCur > max ? max : actualCur;
            GM_setValue("Config.readPoint", finalCur);
            FuckF.notify("🔵", `📖阅读任务已完成！\n✨今日阅读奖励：${finalCur}/${max}`, true, "task.read.done");
        }
        return true;
    } catch (e) {
...
FuckF.taskPromos = async () => {
    ...
    if (totalNewTasks < 1) {
        ...
        if (FuckD.promos.date != FuckD.bing.dateNowNum) {
            ...
            FuckF.notify("🔵", `🧩活动任务已完成！${dashReport}${earnReport}`, true, "task.promos.done");
        }
        return true;
    }
    try {
        ...
        for (const item of promosArr) {
            ...
            if (dashCurrent - 1 + earnCurrent - 1 < totalNewTasks) {
                await new Promise(resolve => setTimeout(resolve, FuckF.getScopeRandomNum(2000, 4000)));
            }
        }
        FuckD.promos.end++;
        if (FuckD.promos.date != FuckD.bing.dateNowNum) {
...
FuckF.taskSearch = async () => {
    ...
    if (FuckD.search.index === 0) {
        ...
        if (restrictedTimes >= 2) {
            ...
            FuckF.notify("🔴", `⚠️积分收入受限或账号异常，已中断今日搜索！${pcReport}${mReport}`, true, "task.search.restricted");
            return true;
        }
    }
        ...
        if (dashboard) {
            ...
            if (realPcPro >= FuckD.search.pc.max && realMPro >= FuckD.search.m.max) {
                ...
                if (FuckD.search.date != FuckD.bing.dateNowNum) {
                    ...
                    FuckF.notify("🔵", `🔍搜索任务已完成！${pcReport}${mReport}`, true, "task.search.done");
                }
                return true;
            } else {
                ...
                if (FuckD.bing.status && ipcc != "CN") {
                    try {
                        ...
                        if (qryResult && FuckF.isJSON(qryResult)) {
                            ...
                            FuckD.bing.ip = (resJSON.code == 0 && resJSON.extra && resJSON.extra.ip) ? resJSON.extra.ip : "";
...
const isTaskDoneForPageRun = (taskEnabled, taskDate) => !taskEnabled || taskDate == FuckD.bing.dateNowNum;
const isAllDoneForPageRun = isTaskDoneForPageRun(FuckD.tasks.sign, FuckD.sign.date) && isTaskDoneForPageRun(FuckD.tasks.read, FuckD.read.date) && isTaskDoneForPageRun(FuckD.tasks.promos, FuckD.promos.date) && isTaskDoneForPageRun(FuckD.tasks.search, FuckD.search.date);
const isBingPageRun = /(^|\.)bing\.com$/i.test(location.hostname);
if (isBingPageRun && isAllDoneForPageRun) {
    if (GM_getValue("Config.allDoneDate", 0) != FuckD.bing.dateNowNum) {
        GM_setValue("Config.allDoneDate", FuckD.bing.dateNowNum);
    }
    FuckF.log("💤", "今日任务已完成，普通 Bing 页面不再启动脚本...");
    return finish();
}
if (FuckD.api.mode != "offline") {
...
FuckF.tasksEnd = async () => {
    if (FuckD.bing.code < 0) {
        if (FuckD.bing.code === -1) {
            FuckD.bing.code = -2;
            FuckF.notify("🔴", `🚫当前 IP 非中国大陆地区，已停止运行！\n⛔IP 地址：${FuckD.bing.ip} - ${FuckD.bing.region}${FuckD.bing.ipInfo}`, true, "mainland.locked");
        }
        return finish();
    }
        ...
        if (FuckD.bing.end === 0) {
            ...
            if (isAllDone) {
                ...
                if (allDoneDate != FuckD.bing.dateNowNum) {
                    ...
                    FuckF.notify("🔵", `🎉今日所选任务已全部完成！\n✨今日共获取：${todayPoints} 积分\n💰 当前总积分：${availablePoints} 积分`, true, "task.all.done");
                } else {
                    await new Promise(resolve => setTimeout(resolve, 1000));
...
FuckF.tasksStart = async (initDelay = 0) => {
    if (GM_info.script.author != "geoisam@qq.com") return finish();
    ...
    if ((!isKeep && isAllDone) || !FuckD.bing.repo.includes("geoisam") || (!FuckD.tasks.sign && !FuckD.tasks.read && !FuckD.tasks.promos && !FuckD.tasks.search)) {
        ...
        //const initDelay = FuckF.getScopeRandomNum(1000, 1000);
        //调试用，1秒停留时间
