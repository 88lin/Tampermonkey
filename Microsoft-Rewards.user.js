...
FuckF.renewToken = async () => {
    ...
    const fetchCodeLogic = async (msg) => {
        const helperState = GM_getValue("Config.helperActive", 0);
        ...
        if (helperState !== -1) {
            ...
            GM_setValue("Config.helperActive", -1);
        } 
        FuckD.bing.sendMSG = `${msg}，已跳过签入和阅读任务，请点击此通知手动获取授权码，或安装授权助手后恢复自动获取`;
...
FuckF.mainlandCheck = async () => {
    try {
            ...
            if (data) {
                ...
                if (FuckD.bing.status && ipcc != "CN") {
                    ...
                    if (qryResult && FuckF.isJSON(qryResult)) {
                        ...
                        FuckD.bing.ip = resJSON.code == 0 ? resJSON.extra.ip : ""
                        FuckD.bing.ipInfo = resJSON.code == 0 ? `\n🌏${resJSON.ipInfo}` : ""
                    }
                    return true
                }
            }
        ...
        FuckF.log("🟡", `网络检测异常，已跳过：${e.message}`);
        return false;
    }
}
...
FuckF.tasksEnd = async () => {
    if (FuckD.bing.code < 0) {
        FuckD.bing.mainland--;
        if (FuckD.bing.mainland < 0) {
            FuckD.bing.sendMSG = `🚫当前 IP 非中国大陆地区，脚本停止运行\n⛔${FuckD.bing.ip}-${FuckD.bing.region}${FuckD.bing.ipInfo}`;
            FuckF.log("🟡", FuckD.bing.sendMSG, true);
            FuckF.send(FuckD.wh);
        }
        return resolve();
